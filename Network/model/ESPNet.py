...
class DownSamplerB(nn.Module):
    def __init__(self, nIn, nOut):
        super().__init__()
        n = int(nOut/5)  #K=5  5条分支，5个不同的扩张率dialted=1，2，4，8，16
        n1 = nOut - 4*n     # 1/5 nOut
        self.c1 = C(nIn, n, 3, 2)  #nIn, nOut, kSize, stride
        self.d1 = CDilated(n, n1, 3, 1, 1) #nIn, nOut, kSize, stride, dilated
    ...
    def forward(self, input):
        ...
        add1 = d2       # d2+d1  ?？add1 = d1 +d2 ?
        add2 = add1 + d4
        add3 = add2 + d8
        add4 = add3 + d16

        combine = torch.cat([d1, add1, add2, add3, add4],1)
        #combine_in_out = input + combine     #shotcut path 跳过连接
...
class DilatedParllelResidualBlockB(nn.Module):
    ...
    def __init__(self, nIn, nOut, add=True):
        ...
        '''
        super().__init__()
        n = int(nOut/5)  #K=5, 用于生成channel = nOut的输出特征图
...
class ESPNet_Encoder(nn.Module):
    ...
    def __init__(self, classes=19, p=5, q=3):
        ...
        '''
        super().__init__()                  #def __init__(self, nIn, nOut, kSize, stride):
    ...
    def forward(self, input):
        ...
        for i, layer in enumerate(self.level2): #enumerate()(单词意思是枚举的意思),函数中的参数X可以是一个迭代器(iterator)或者是一个序列，
            if i==0:
        ...
        for i, layer in enumerate(self.level3):
            if i==0:
                output2 = layer(output2_0) #第一个module输入为output2_0,下面的module输入为output2
            else:
...
class ESPNet(nn.Module):
    ...
    def __init__(self, classes=19, p=2, q=3, encoderFile=None):
        ...
        self.en_modules = []  #加载编码模块
        for i, m in enumerate(self.encoder.children()): #self.children()只包括网络模块的第一代儿子模块；self.modules()包含网络模块的自己本身和所有后代模块
            self.en_modules.append(m)

        # light-weight decoder  轻量级解码
        self.level3_C = C(128 + 3, classes, 1, 1)
        self.br = nn.BatchNorm2d(classes, eps=1e-03)
        self.conv = CBR(19 + classes, classes, 3, 1) #def __init__(self, nIn, nOut, kSize, stride=1):
                     #建立nn.Sequential()对象，必须小心确保一个块的输出大小与下一个块的输入大小匹配
...
if __name__ == '__main__':
    ...
    model = ESPNet(classes=11).to(device)
